...
template <size_t NBLAYERS, size_t INPUTNUMBER, size_t OUTPUTNUMBER, size_t TRAINING_LENGTH, size_t POP_SIZE>
std::function<float(std::array<Matrix <float, OUTPUTNUMBER, 1>, TRAINING_LENGTH>, std::array< Matrix <float, OUTPUTNUMBER, 1> , TRAINING_LENGTH>)> GeneticAlgorithm<NBLAYERS, INPUTNUMBER, OUTPUTNUMBER, TRAINING_LENGTH, POP_SIZE>::ERRORS_COUNT = [] (std::array<Matrix <float, OUTPUTNUMBER, 1>, TRAINING_LENGTH> training_set, std::array< Matrix <float, OUTPUTNUMBER, 1> , TRAINING_LENGTH> real_outputs) {
    float errors(0);
    for (std::size_t inputid(0); inputid < TRAINING_LENGTH; inputid++){
        for (std::size_t outputIncr(0); outputIncr < OUTPUTNUMBER; outputIncr++){
